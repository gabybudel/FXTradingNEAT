    ...
    for genome_id, genome in genomes:
        ...
        for d in train_dates:
            ...
            if len(complete_crncy) >= 3:
                ...
                if np.min(activations['Activation']) == np.max(activations['Activation']):
                    if np.sum(long == short) == len(long):
                        order = np.random.permutation([d for d in range(len(long))])
                        long = activations.index[order[0:n_invest]]
                        short = activations.index[order[n_invest:(n_invest*2)]]
                    else:
                        print('Unexpected activations: ' + str(activations['Activation']))
                ml_signals.loc[d,long] = 1
                ml_signals.loc[d,short] = -1
        ...
        avg_ret = (12*np.mean(returns))
        if len(returns) == 0:
            print('Error: ' + str(avg_ret))
        if str(avg_ret) == 'nan':
            avg_ret = -999
        genome.fitness += avg_ret
